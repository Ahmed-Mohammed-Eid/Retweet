...
export default function SelectSubcategoryItem({lang}) {
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.BASE_URL}/user/subcategories`, {
            ...
            .then(response => {
                ...
                const selectedCategory = categories.find(category => category._id === subCategoryId);
                ...
                setFilteredCategories(lang === 'en' ? selectedCategory.itemsEn : selectedCategory.items);
            ...
            <div className={`${classes.Content}`}>
                <div className={`${classes.CategoriesSearch}`}>
                    ...
                    <input
                        ...
                        onChange={(event) => {
                            ...
                            if (search.length === 0) return setFilteredCategories(lang === 'en' ? items.itemsEn : items.items);

                            const filteredCategories = lang === 'en' ? items.itemsEn.filter(category => category.toLowerCase().includes(search.toLowerCase())) : items.items.filter(category => category.toLowerCase().includes(search.toLowerCase()));
                            // SET THE CATEGORIES
                ...
                </div>
                {
                    filteredCategories.map((category, index) => {
                        return (
                            <CategoryItem
                                key={`${lang === 'en' ? (items.itemsEn[index] + '-' + index) : (items.items[index] + '-' + index)}`}
                                hasNoImage={true}
                                category={
                                    lang === 'en' ? items.itemsEn[index] : items.items[index]
                                }
                                icon={'/assets/listings/RightArrow.svg'}
                                clicked={() => {
                                    router.push(`/listings/select-images?category=${categoryIdFromUrl}&subCategory=${subCategoryId}&formType=${formType}&item=${lang === 'en' ? items.itemsEn[index] : items.items[index]}`)
                                }}
                            />
                        )
                    })
