...
export default function SelectCategory({lang}) {
                        ...
                        onChange={(event) => {
                            ...
                            const filteredCategories = categories.filter(category => {
                                return category.categoryName.toLowerCase().includes(search.toLowerCase()) ||
                                    category.categoryNameEn.toLowerCase().includes(search.toLowerCase());
                            })
                            // SET THE CATEGORIES
                    ...
                    filteredCategories.map((category) => {
                        return (
                            <CategoryItem
                                ...
                                category={
                                    lang === 'en' ? category.categoryNameEn : category.categoryName
                                }
                                icon={'/assets/listings/RightArrow.svg'}
                                clicked={() => {
                                    router.push(`/listings/select-subcategory?category=${category._id}`)
                                }}
                            />
                        )
                    })
