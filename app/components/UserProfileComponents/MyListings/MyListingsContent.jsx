...
// IMPORTS
import EditCard from "@/app/components/Listings/EditCard/EditCard";
...
export default function MyListingContent({lang}) {

    // REDUX
    const isAuthenticated = useSelector(state => state.mainLayout.isAuthenticated);
    ...
    async function deleteListing() {
        ...
        await axios.delete(`${process.env.BASE_URL}/delete/user/listing?listingId=${deleteDialog.id}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
            .then(response => {
                setMyListings(myListings.filter(listing => listing._id !== deleteDialog.id))
                setDeleteDialog({
                    visible: false,
                    id: null
                })
                toast.success(lang === 'en' ? 'Listing deleted successfully' : 'تم حذف الإعلان بنجاح')
            })
            .catch(error => {
                console.log(error)
                toast.error(lang === 'en' ? 'An error occurred while deleting the listing' : 'حدث خطأ أثناء حذف الإعلان')
            })
    }
    ...
    return (
        <div className={'w-full min-h-screen bg-white flex flex-col gap-2 py-6'}>
            <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {myListings.length === 0 && (
                    <div className="w-full flex flex-col items-center justify-center">
                        <h1 className="text-2xl font-bold text-gray-500">
                            {lang === 'en' ? 'No Listings Found' : 'لم يتم العثور على إعلانات'}
                        </h1>
                    </div>
                )}
                ...
                {/*  DELETE DIALOG  */}
                <Dialog
                    header={lang === 'en' ? 'Delete Listing' : 'حذف الإعلان'}
                ...
                >
                    <div className="p-6">
                        <h1 className="text-lg font-bold text-gray-500 md:text-2xl">
                            {lang === 'en' ? 'Are you sure you want to delete this listing?' : 'هل أنت متأكد أنك تريد حذف هذا الإعلان؟'}
                        </h1>
