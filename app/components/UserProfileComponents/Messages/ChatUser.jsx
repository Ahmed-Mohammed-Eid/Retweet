...
import {updateCreatorId, updateSelectedUser, updateMessages} from "@/redux/Slices/chatSlice";
...
export default function ChatUser({
    ...
    const {userId, creatorId, selectedUser} = useSelector(state => state.chat);

    const active = creatorId === selectedUser.id && creatorId !== "";

    // GET THE CHAT
    function getChat(partnerId) {
        const token = localStorage.getItem("retweet-token");

        axios.get(`${process.env.BASE_URL}/messages/history?partnerId=${partnerId}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        }).then(response => {
            dispatch(updateMessages(response?.data?.incomingMessages || []));
        }).catch(error => {
            console.log(error);
    ...
    return (
        <section className={[classes.chat_user, active ? classes.active : ''].join(' ')}
                 data-id={userId}
                 onClick={() => {
                     dispatch(updateCreatorId(userId === user.senderUserId ? user.receiverUserId : user.senderUserId));
                     dispatch(updateSelectedUser({
                         id: userId === user.senderUserId ? user.receiverUserId : user.senderUserId,
                         fullName: user.senderDetails?.fullName || "Unknown",
                     }));
                     getChat(userId === user.senderUserId ? user.receiverUserId : user.senderUserId);
                 }}
        >
            <div className={classes.chat_user_top}>
                <h4 className={classes.chat_user_name}>
                    {(!user?.isRead && !active) &&
                        (<span className={classes.has_new_message}>
                        !
                    </span>)}
                    {user?.senderDetails?.fullName || "Unknown"}
                </h4>
                <span className={classes.chat_user_time}>
                    {time}
            </span>
            </div>
            <div className={classes.chat_user_bottom}>
                ...
                <p className={classes.chat_user_message}>
                    {user.message}
                </p>
            </div>
        </section>
